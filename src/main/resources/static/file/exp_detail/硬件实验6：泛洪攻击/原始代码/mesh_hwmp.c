...
//ymj
 	if(test_bit(MESH_WORK_BH_ATTACK,&sdata->u.mesh.wrkq_flags))
 		return;
//ymj
	if (memcmp(target_addr, sdata->vif.addr, ETH_ALEN) == 0)
...
void mesh_rx_path_sel_frame(struct ieee80211_sub_if_data *sdata,
			    ...
			    size_t len)
{
	struct ieee80211_if_mesh *ifmsh=&sdata->u.mesh;	//ymj
	...
	if (elems.preq) {
		...
		if (last_hop_metric)
 		{//ymj
 			...
 			if(test_bit(MESH_WORK_BH_ATTACK,&ifmsh->wrkq_flags))
 				hwmp_preq_frame_bh_process(sdata, mgmt, elems.preq,
 						last_hop_metric);
 			else//ymj
