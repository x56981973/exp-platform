...
void ieee80211s_stop(void)
{
	...
	//to be modified
	//取消bl（blacklist）的注册，调用mesh_def.c中的函数
...
void ieee80211_mesh_init_sdata(struct ieee80211_sub_if_data *sdata)
{
	...
	ifmsh->sn = 0;
	ifmsh->num_gates = 0;
	//if (ifmsh->mshcfg.dot11MeshHWMPRootMode == 2)
 		set_bit(MESH_WORK_FL_DEFENCE,&ifmsh->wrkq_flags);//ymj
	//else if (ifmsh->mshcfg.dot11MeshHWMPRootMode == 3)
 		//set_bit(MESH_WORK_BH_DEFENCE,&ifmsh->wrkq_flags);
 	...
 	ifmsh->last_broadcast_bl = jiffies;
  	...
  	if (!mesh_allocated)
  		ieee80211s_init();
 	if(test_bit(MESH_WORK_FL_DEFENCE,&ifmsh->wrkq_flags)||
 			test_bit(MESH_WORK_BH_DEFENCE,&ifmsh->wrkq_flags))
 		mesh_bl_table_init();
 	mesh_ids_set_default(ifmsh);//ymj
