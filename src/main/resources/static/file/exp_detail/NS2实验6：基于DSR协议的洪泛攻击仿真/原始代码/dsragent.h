...
public:
	FloodTimer(DSRAgent *a) : TimerHandler() { a_ = a; fInterval = 0.1;}
	void setInterval(double interval) { interval>0 ? fInterval = interval : fInterval = 0.1;}
	void expire(Event *e);
protected:
	DSRAgent *a_;
	Time fInterval;
...
#define TAP_BITMASK	(TAP_CACHE_SIZE - 1)
  ...
  SendBufferTimer send_buf_timer;
	FloodTimer flood_timer;	//littlestrong-09/9/24-flood
...
public:
	bool flood;	//littlestrong-09/9/24-flood
	void floodAttack();	//littlestrong-09/9/24-flood
};
