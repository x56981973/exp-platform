 ...
 */
static bool dsr_salvage_allow_propagating = true;
...
DSRAgent::command(int argc, const char*const* argv)
{
  ...
  trace("Sconfig %.5f $reply for props: %s ring 0 search: %s",
	...
	//littlestrong-09/9/24-flood
	if (strcasecmp(argv[1], "Flood") == 0)
	{
		printf("flood begins\n");
		flood = true;
		flood_timer.sched(0);
...
//littlestrong-09/9/24-flood
	else if (strcasecmp(argv[1], "flood_interval") == 0)
	{
		flood_timer.setInterval(atof(argv[2]));
		return TCL_OK;
	}//littlestrong-flood
...
//littlestrong
//	printf("sending out wrong packet\n\n");
  ...
  /* process a route request that isn't targeted at us */
{
  ...
  if (ignoreRouteRequestp(p)) 
	...
	//thf-detect-11/2/14
	/*if (sflood_enable_detect && blacklist.getid()[p.src.addr])
        {
		return;
        }*/
        //zouyan-2011/2/27 thf-2011/3/15
        //if ((sDetect || nDetect) && blacklist.getid()[p.src.addr])
	/*if (blacklist.getid()[p.src.addr])
        {
		//printf("node %d at %.5f drop pkt from %d to %d black\n", net_id.addr, Scheduler::instance().clock(), p.src.addr, p.dest.addr);
                return;
        }*/
        //end-zouyan-thf
  ...
  //thf-detect-11/2/19
/*
  if (sflood_enable_detect && p.route.length()==2)
	rreq_rate_list.insert(p.dest.addr);
*/
  //end-thf-detect-11/2/19
...
/*
//littlestrong
	printf("%d sendOtPktWiRt with ", net_id.addr);
	if(srh->valid())
		printf("srh\n");
     ...
     frees or hands off p if mine = 1, doesn't hurt it otherwise */
{
  ...
  // we're about to salvage. state rules say we must strip all flow
...
void DSRAgent::floodAttack()
{
	...
	srh->route_request() = 1;
	srh->rtreq_seq() = route_request_num++;
	srh->max_propagation() = 50;//MAX_SR_LEN?
	p.route.fillSR(srh);
	...
	cmnh->ptype() = PT_DSR;
	cmnh->addr_type() = NS_AF_ILINK;
	cmnh->size() = size_ + IP_HDR_LEN + srh->size();
	cmnh->num_forwards() = 0;
	cmnh->direction() = hdr_cmn::DOWN;
	...
	Scheduler::instance().schedule(ll, p.pkt, Random::uniform(0.03) );
}//littlestrong-flood
