...
{
	struct mesh_bl_node *node = mesh_bl_lookup(addr,true,sdata);
	int ret;
	if(!node)
		ret = MESH_BL_NEXIST;
	else
	...
	{
		case MESH_BL_NEXIST:
			mesh_bl_node_add(addr,flood_nodes,sdata);
			break;
		case MESH_BL_ACTIVE:
			if(atomic_inc_return(&node->count) >= DETECT_THRESHOLD)
			{
				mdef_dbg("%pM is Black,suppress it!\n",addr);
				spin_lock_bh(&node->state_lock);
				node->state = MESH_BL_SUPPRESSED;
				ret = node->state;
				spin_unlock_bh(&node->state_lock);
				atomic_set(&node->count,0);
			}
			break;
		case MESH_BL_SUPPRESSED:
			break;
		default:
			break;
	}
...
int mesh_bh_detect(u8 *addr,struct ieee80211_sub_if_data *sdata)
{
	struct mesh_bl_node *node；
	//tobemodified//查找是否在黑名单中
	int ret;//声明返回值变量
	if(!node)//若不在黑名单中
		//tobemodified
		//标为不存在的节点
